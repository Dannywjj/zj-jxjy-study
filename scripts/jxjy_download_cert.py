...
交付物口径（已与用户确认 2026-09-14）：
  ...
  2026 学习计划 ID 实测为 21037（isContinueLearning(...,21037,...) 与 09-12 成功拉证一致）。
...
行为：
  - 直连 chinaacc /mgt/certPrint?studyID=21037（认 jxjy_state.json 的 mgt 会话）。
...
# 正保学习计划 ID（2026 实测）。若平台变更，可在此调整或后续改为动态抓取。
STUDY_ID = "21037"
CERTPRINT_URL = f"https://jxjy.chinaacc.com/mgt/certPrint?studyID={STUDY_ID}"
